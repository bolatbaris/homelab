...
- Implemented services in this tier: the `env` profile's Infisical (`infisical`), the `db` profile's PostgreSQL (`appdb`) and Adminer (`appdb-adminer`), the `analytics` profile's Umami (`umami`), and the `monitoring` profile's GlitchTip (`glitchtip`). They bind the tailscale0 address - the `db` profile through compose's required-variable form, so an empty address fails the render instead of falling back to every interface; `install.sh` refuses all four profiles unless `TAILSCALE_ENABLED=true`, probes that podman-compose actually implements that form, and CI asserts the behaviors. Infisical, Umami, and GlitchTip additionally fall back to loopback rather than a wildcard bind if `TAILSCALE_IP` is ever empty.
...
openssl rand -hex 32      # GLITCHTIP_SECRET_KEY and GLITCHTIP_DB_PASSWORD if enabling --profile monitoring
```
...
- The same applies to `monitoring`: `./data/glitchtip` holds the live GlitchTip database plus logical dumps, which carry the monitoring user-table password hashes.
