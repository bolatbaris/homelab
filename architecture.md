...
- **Tier B (Private)** - reachable only through Tailscale, bound to the tailscale0 address: the `env` profile's Infisical, the `db` profile's PostgreSQL (`appdb`) and Adminer (`appdb-adminer`), and the `analytics` profile's Umami. Gitea SSH moves here from its LAN bind later.
- **Tier C (Internal)** - never published: mattermost-postgres, infisical-postgres, umami-postgres, the `appdb-dump`, `infisical-db-dump`, and `umami-db-dump` sidecars, backup (`network_mode: none`), Portainer (mgmt profile).
...
2. When `analytics` is enabled, write a logical Umami PostgreSQL dump to `./data/umami/db-dumps`.
3. When `chat` is enabled, write a logical Mattermost PostgreSQL dump to `./data/mattermost/db-dumps`.
4. When `env` is enabled, write a logical Infisical PostgreSQL dump to `./data/infisical/db-dumps`.
5. When `db` is enabled, write `pg_dumpall --globals-only` plus one logical dump per application database to `./data/appdb/db-dumps`.
6. Initialize restic at `${RESTIC_REPOSITORY}` if needed.
7. Snapshot `/sources`.
8. Apply retention with daily, weekly, and monthly keep counts.
